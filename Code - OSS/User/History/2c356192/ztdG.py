...
while (True):
    ...
    if(cons == '7'):
        ...
        try:
            consulta = "SELECT r.customer_id, p.product_group, COUNT(r.customer_id) AS qtd_reviews"
            consulta += "FROM review AS r"
            consulta += "JOIN product AS p ON r.asin_product = p.asin"
            consulta += "WHERE p.product_group IN ('Book', 'Video Games', 'Sports', 'DVD', 'Baby Product', 'Video', 'Music', 'Software', 'CE', 'Toy')"
            consulta += "GROUP BY r.customer_id, p.product_group"
            consulta += "ORDER BY p.product_group, qtd_reviews DESC"
            consulta += "LIMIT 10"
