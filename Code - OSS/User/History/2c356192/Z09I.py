...
while (True):
    ...
    if(cons == '10'):
        ...
        try:
            ...
            consulta += "UNION (SELECT r.id_cliente,p.grupo,count(r.id_cliente) "
            consulta += "AS qtd_reviews FROM reviews as r JOIN produtos as p ON r.asin_prod = p.asin where asin_prod IN "
            consulta += "(SELECT asin FROM product where product_group='Toy') GROUP BY r.customer_id, p.product_group ORDER BY qtd_reviews DESC LIMIT 10)) ORDER BY product_grup, qtd_reviews DESC"
            cur.execute(consulta)
