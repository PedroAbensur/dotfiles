...
for _ in range(int(items)):
    ...
    if(line[0] == 'discontinued'):
        ...
        for r in range(n_rev):
            ...
            rev_list = line.split()
            if len(rev_list) == 1: # new product
                asin_product = rev_list[0]
            else: # new review for current product
                customer_id = rev_list[0]
                review_date = rev_list[1]
                rating = rev_list[2]
                votes = rev_list[3]
                helpful = rev_list[4]
            rev = review(asin_product, customer_id, review_date, rating, votes, helpful)
            rev.print_reviews()
