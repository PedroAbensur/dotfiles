...
for _ in range(int(items)):
    ...
    if(line[0] == 'discontinued'):
        ...
        rev_split = rev.split()
        n_rev = int(rev_split[1])
        for r in range(n_rev):
            line = f.readline().strip() #linha das reviews
            review_data = line.split()
            asin_product = review_data[0]
            customer_id = review_data[2]
            review_date = review_data[1]
            rating = int(review_data[4])
            votes = int(review_data[6])
            helpful = int(review_data[8])
            review = Review(asin_product, customer_id, review_date, rating, votes, helpful)
            rev.print_reviews()
